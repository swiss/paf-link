...
Activities can use **input entities** that provide the necessary data that the activity acts upon. Activities can generate **output entities** representing the result of the activity. But also without generating output entities, the sole fact that an activity happened can also signal some information (e.g. an acknowledgment activity means that something is acknowledged even without generating an output entity).
...
A special case is an **identifier entity** that represent the identifier of a specific public affair. Such identifier entities should not contain any other information than the identifier which should also be part of the URI of such entities. Every activity that acts upon a specific affair should use the corresponding identifier entity as part of the input entities.

### Entities in Proposal and Decision Activities

If an affair consists of something like a proposal and a decision, the sum of all prov:used entities of the proposal activity form together *the proposal*. This proposal activity can only be used to group already existing entities (an entity should not be linked with prov:used and prov:wasGeneratedBy to the same activity). If a proposal needs additional entities, they have to be generated by some kind of proposal preparation activity before. The decision entity again lists all the entities it decides upon - they do not have to be the same as in the proposal if the deciding agent(s) change the proposal before the decision. See also [Basic Affair](#basic-affair).
