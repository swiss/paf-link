...
Activities can use **input entities** that provide the necessary data that the activity acts upon. Activities can generate **output entities** representing the result of the activity. But also without generating output entities, the sheer fact that an activity happened can also signal some information (e.g. an acknowledgment activity means that something is acknowledged even without generating an output entity). 

If an activity is meant to decide on a proposal, the proposal is the sum of the input entities for this decision activity. These input entities can be generated as output entities of a decision preparation activity.
...
<figure id="design_principles_change">
  ...
  <figcaption>
    A change of an existing public affair.
  </figcaption>
</figure>
